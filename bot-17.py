...
async def yt_quality_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_audio:
        try:
            ...
            d = r.json()
            logger.info(f"[yt-api audio] response keys: {list(d.keys())}")
            logger.info(f"[yt-api audio] full response: {str(d)[:500]}")
            dl_url = None
        ...
        try:
            ...
            r = requests.get("https://yt-api.p.rapidapi.com/dl",
                             headers=headers, params={"id": url}, timeout=60)
            d = r.json()
            logger.info(f"[yt-api video] response keys: {list(d.keys())}")
            logger.info(f"[yt-api video] full response: {str(d)[:500]}")
            dl_url = None
            # پیدا کردن بهترین فرمت ویدیو با کیفیت انتخابی
            formats = d.get("formats", [])
            if not formats:
                formats = d.get("adaptiveFormats", [])
            if not formats and d.get("url"):
                dl_url = d.get("url")
            # اول دنبال کیفیت دقیق بگرد
            for fmt in formats:
                q = str(fmt.get("qualityLabel", ""))
                if fmt.get("hasVideo") and fmt.get("hasAudio") and str(height) in q:
                    dl_url = fmt.get("url")
                    break
            # اگه پیدا نشد، هر فرمتی با ویدیو و صدا
            if not dl_url:
                for fmt in formats:
