...
def init_db():
    with db() as con:
        ...
        INSERT OR IGNORE INTO settings VALUES ('quiz_required_invites', '2');
        """)
...
# ══════════════════════════════════════════════════════════════════════════
QUIZ_NEEDED = 5  # تعداد سوال هر آزمون
...
async def quiz_create_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.answer()
    user = query.from_user
    user_quiz_state[user.id] = {"step": 0, "questions": []}
    await query.message.reply_text(
        "✏️ *بساز آزمونت رو!*\n\n"
        "۵ تا سوال درباره خودت طرح کن (هر چی باشه: علایق، خاطرات، عادت‌ها...)\n"
        "بعد از هر سوال، جواب درستش رو هم بفرست.\n\n"
        "مثال سوال:\n"
        "_رنگ مورد علاقه من چیه؟_\n\n"
        "📝 *سوال ۱ از ۵* رو بنویس:",
        parse_mode="Markdown"
    )
...
async def handle_quiz_creation_text(update, context, text):
    ...
    if awaiting == "answer":
        ...
        await update.message.reply_text(
            f"📝 *سوال {state['step']+1} از {QUIZ_NEEDED}* رو بنویس:",
            parse_mode="Markdown"
        )
        return True
...
async def send_quiz_results(message, context, quiz_id, owner_id):
    ...
    if n < required:
        ...
        await message.reply_text(
            ...
            f"فقط *{n} نفر* تا الان شرکت کرده‌اند.\n\n"
            f"📣 *{remaining} نفر دیگه* دعوت کن تا نتیجه کامل و رتبه‌بندی دوستات باز شه!\n\n"
...
async def enter_quiz_flow(update, context, quiz_id):
    ...
    user_quiz_play[user.id] = {"quiz_id": quiz_id, "index": 0, "score": 0, "owner_name": quiz["owner_name"]}
...
async def handle_quiz_play_text(update, context, text):
    ...
    if state["index"] >= len(questions):
        ...
        bot_username = await get_bot_username(context)
        my_link_user = update.message.from_user
        keyboard = [
        ...
        await update.message.reply_text(
            f"🎉 *امتیاز شما:*\n\n"
            f"{score} از {total}\n"
            f"📊 {percent}٪ شناخت از {owner_name}\n\n"
            f"حالا نوبت توئه! ببین دوستات چقدر تو رو می‌شناسن 😏",
            parse_mode="Markdown",
            reply_markup=InlineKeyboardMarkup(keyboard)
        )
        ...
        if quiz:
            try:
                ...
                if len(results) >= required:
                    extra = "\n\n📊 نتیجه کامل آزمونت الان باز شده! برای دیدنش روی «📊 نتایج من» بزن."
                await context.bot.send_message(
                    quiz["owner_id"],
                    f"🔔 *{user.first_name or user.username or 'یک دوست'}* در آزمونت شرکت کرد!\n"
                    f"امتیاز: {score} از {total} ({percent}٪){extra}",
                    parse_mode="Markdown"
                )
            except Exception:
                pass
