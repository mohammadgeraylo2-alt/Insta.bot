...
async def shazam_detect_from_file_async(input_path):
    ...
    try:
        ...
        # ۱) اول فایل اصلی رو مستقیم بده به shazam (نیازی به ffmpeg نیست)
        try:
            ...
            logger.warning(f"shazamio direct recognize failed: {e}")

        # ۲) اگه نشد، با ffmpeg تبدیل کن و دوباره امتحان کن
