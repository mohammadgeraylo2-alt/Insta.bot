...
async def send_quiz_results(message, context, quiz_id, owner_id):
    ...
    if invite_count < required:
        ...
        await message.reply_text(
            ...
            f"فقط *{invite_count} نفر* تا الان با لینکت وارد شدن.\n\n"
    ...
    if not results:
        bot_username = await get_bot_username(context)
        link = f"https://t.me/{bot_username}?start=quiz_{quiz_id}"
        keyboard = [[InlineKeyboardButton("📤 دعوت دوستان", url=f"https://t.me/share/url?url={link}&text=ببین چقدر می‌شناسیم 😏")]]
        await message.reply_text(
            "✅ *نتیجه باز شده!*\n\nهنوز کسی آزمونت رو کامل نکرده.\nلینک رو بیشتر بفرست! 🚀",
            parse_mode="Markdown",
            reply_markup=InlineKeyboardMarkup(keyboard)
        )
        return
