...
async def shazam_detect(file_path):
    try:
        shazam = Shazam()
...
async def handle_link(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ─── اینستاگرام استوری ───────────────────────────────────
    if "instagram.com" in text and "/stories/" in text:
        msg = await update.message.reply_text("⬇️ دارم استوری رو دانلود میکنم...")
        video_path = f"story_{user_id}.mp4"
        try:
            ...
            if isinstance(data_r, dict):
                media = data_r.get("media", [])
                if media: video_url = media[0].get("url")
                if not video_url:
                    video_url = data_r.get("url") or data_r.get("video_url") or data_r.get("download_url")
            elif isinstance(data_r, list) and data_r:
                video_url = data_r[0].get("url") or data_r[0].get("video_url")
            ...
            if result and result.get("title"):
                ...
                if downloaded:
                    ...
                    await msg.edit_text(f"🎵 *{title} - {artist}*\n\n❌ دانلود آهنگ ممکن نشد، اسمش رو مستقیم بنویس.", parse_mode="Markdown")
            else:
