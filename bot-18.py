...
GENERATE_MODEL = "stabilityai/stable-diffusion-xl-base-1.0"
...
# ─── ساخت و ادیت عکس با Hugging Face ──────────────────────────────────────
def hf_request(api_url, payload_json=None, files=None, form_data=None, retries=3):
    for attempt in range(retries):
        try:
            if files:
                r = requests.post(api_url, headers=HF_HEADERS, files=files, data=form_data, timeout=120)
            else:
                r = requests.post(api_url, headers=HF_HEADERS, json=payload_json, timeout=120)
            if r.status_code == 200:
                return r.content, None
            elif r.status_code == 503:
                est = r.json().get("estimated_time", 20)
                logger.info(f"HF مدل لود میشه، {est:.0f}s صبر...")
                time.sleep(min(est + 5, 60))
            ...
            return None, str(e)
    return None, "بعد از چند تلاش موفق نشد"

async def ai_generate_image(prompt):
    data, err = hf_request(
        f"https://api-inference.huggingface.co/models/{GENERATE_MODEL}",
        payload_json={"inputs": prompt, "parameters": {"num_inference_steps": 30, "guidance_scale": 7.5}}
    )
    return data, err

async def ai_edit_image(img_bytes, instruction):
    pil = Image.open(io.BytesIO(img_bytes)).convert("RGB").resize((512, 512))
    buf = io.BytesIO()
    pil.save(buf, format="PNG")
    buf.seek(0)
    data, err = hf_request(
        f"https://api-inference.huggingface.co/models/{EDIT_MODEL}",
        files={"image": ("image.png", buf, "image/png")},
        form_data={"prompt": instruction, "num_inference_steps": 20}
    )
    return data, err
