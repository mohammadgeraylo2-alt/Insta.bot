        ...
        if status == "success":
            ...
            for _ in range(30):
                ...
                try:
                    ...
                    pass
        return None, data.get("message", "نتیجه‌ای دریافت نشد")
    except Exception as e:
        return None, str(e)
...
# ─── ساخت ویدیو با Hugging Face (fallback رایگان) ────────────────────────
async def hf_text2video(prompt):
    """ساخت ویدیو از پرامپت با Hugging Face - مدل AnimateDiff"""
    if not HF_TOKEN:
        return None, "کلید HF_TOKEN تنظیم نشده."
    try:
        headers = {
        ...
        # مدل AnimateDiff برای text2video
        r = requests.post(
        ...
        if r.status_code == 200 and len(r.content) > 10000:
            return r.content, None
        # اگه مدل در حال لود بود، صبر کن و دوباره امتحان کن
        ...
        return None, f"HuggingFace خطا: {r.status_code} - {r.text[:150]}"
    except Exception as e:
        return None, str(e)


async def hf_img2video(img_bytes, prompt=""):
    """تبدیل عکس به ویدیو با Hugging Face - مدل Stable Video Diffusion"""
    if not HF_TOKEN:
        return None, "کلید HF_TOKEN تنظیم نشده."
    try:
        headers = {"Authorization": f"Bearer {HF_TOKEN}"}
        # ریسایز عکس به 1024x576 برای SVD
        pil = Image.open(io.BytesIO(img_bytes)).convert("RGB")
        ...
        if r.status_code == 503:
            try:
                ...
                wait = 30
            await asyncio.sleep(min(wait, 40))
            r2 = requests.post(
                "https://api-inference.huggingface.co/models/stabilityai/stable-video-diffusion-img2vid-xt",
                headers=headers,
                files={"image": ("image.png", img_data, "image/png")},
...
async def handle_link(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if vid_state.get("mode") == "text2video":
        ...
        # اگه ModelsLab کردیت نداشت یا خطا داد، HuggingFace امتحان کن
        if (not vid_data) and HF_TOKEN:
            await msg.edit_text("🔄 سرویس اصلی در دسترس نیست، از سرویس جایگزین استفاده می‌کنم...\n⏳ کمی بیشتر صبر کن 🙏")
            vid_data, err = await hf_text2video(enhanced)
    ...
    if vid_state.get("mode") == "img2video":
        ...
        if (not vid_data) and HF_TOKEN:
            await msg.edit_text("🔄 سرویس اصلی در دسترس نیست، از سرویس جایگزین استفاده می‌کنم...\n⏳ کمی بیشتر صبر کن 🙏")
            vid_data, err = await hf_img2video(img_bytes, prompt)
        if err or not vid_data:
            await msg.edit_text(f"❌ خطا: {err or 'نتیجه‌ای نگرفتم'}\n\nدوباره امتحان کن.")
            return
        await msg.delete()
