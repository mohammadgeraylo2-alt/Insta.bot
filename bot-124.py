...
async def yt_download_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    user_id = query.from_user.id
    await query.answer()
    data = query.data
    yt_data = user_yt_data.get(user_id)
    if not yt_data:
        await query.message.reply_text("❌ لینک پیدا نشد، دوباره بفرست."); return

    url = yt_data["url"]
    title = yt_data["title"]
    cookies_path = yt_data.get("cookies")
    ...
    try:
        if data == "yt_audio":
            ...
            if size_mb > 50:
                await msg.edit_text(f"❌ فایل {size_mb:.0f}MB هست. تلگرام تا 50MB قبول میکنه. کیفیت پایین‌تر انتخاب کن.")
                return
            caption = get_setting("caption") or CAPTION
        ...
        await msg.edit_text("❌ خطا در دانلود:\n" + str(e)[:300])
    finally:
...
async def handle_link(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ─── یوتیوب ────────────────────────────────────────────────
        ...
        try:
            ...
            user_yt_data[user.id] = {"url": text, "title": title, "cookies": cookies_path}
            ...
            for f in reversed(formats):
                height = f.get("height")
                ext = f.get("ext")
                if not height or ext not in ("mp4", "webm"): continue
                label = f"{height}p"
                if label in seen: continue
                seen.add(label)
                video_buttons.append(
                    InlineKeyboardButton(f"📹 {label}", callback_data=f"yt_video_{height}")
                )
            ...
            keyboard.append([InlineKeyboardButton("🎵 فقط صدا (MP3)", callback_data="yt_audio")])
...
app.add_handler(CallbackQueryHandler(yt_download_callback, pattern="^yt_"))
