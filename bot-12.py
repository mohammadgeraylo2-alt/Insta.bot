...
async def yt_quality_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_audio:
        ...
        for api in mp3_apis:
            try:
                yt_headers = {"x-rapidapi-key": RAPIDAPI_KEY, "x-rapidapi-host": api["host"]}
                r = requests.get(api["url"], headers=yt_headers, params={"url": url}, timeout=60)
                data_r = r.json()
                download_url = data_r.get("url") or data_r.get("download_url") or data_r.get("link")
                if download_url:
                    break
                last_err = data_r
            except Exception as e:
                ...
                continue
        if not download_url:
            await msg.edit_text(f"❌ دانلود صدا ممکن نشد: {last_err}")
            return
        try:
            keyboard = [[InlineKeyboardButton("کانال ما 📢", url=f"https://t.me/{clean_ch}")]]
        ...
        try:
            ...
            r = requests.get(
                "https://all-media-downloader4.p.rapidapi.com/youtube",
                headers=yt4_headers,
                params={"url": url, "quality": f"{height}p"},
                timeout=60
            )
            data_r = r.json()
            if isinstance(data_r, list) and data_r:
                download_url = data_r[0].get("url") or data_r[0].get("link") or data_r[0].get("download")
            elif isinstance(data_r, dict):
                medias = data_r.get("medias") or data_r.get("media") or data_r.get("formats") or []
                if isinstance(medias, list):
                    for m in medias:
                        q = str(m.get("quality") or m.get("resolution") or "")
                        if height in q and "mp4" in str(m.get("ext") or m.get("format") or "mp4").lower():
                            download_url = m.get("url") or m.get("link")
                            break
                    if not download_url and medias:
                        download_url = medias[0].get("url") or medias[0].get("link")
                if not download_url:
                    download_url = data_r.get("url") or data_r.get("download_url") or data_r.get("link") or data_r.get("mp4")
            last_err = data_r
        except Exception as e:
            last_err = str(e)
        if not download_url:
            await msg.edit_text(f"❌ دانلود ویدیو ممکن نشد: {last_err}")
            return
        try:
            keyboard = [
