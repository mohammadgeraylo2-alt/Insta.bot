...
async def yt_quality_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_audio:
        ...
        try:
            ...
            r = requests.post(
                "https://cobalt-social-media-downloader.p.rapidapi.com/cobalt-t-download/",
            ...
            video_url = d.get("url") or d.get("link") or d.get("download_url") or d.get("downloadUrl")
            if not video_url and isinstance(d, dict):
                for k, v in d.items():
            ...
            if not video_url:
                raise Exception(f"لینک ویدیو پیدا نشد - {d}")
