...
def normalize_cookie_text(raw: str) -> str:
    """
    Variable های تک‌خطی Railway معمولاً newline واقعی رو حذف می‌کنن.
    این تابع اجازه می‌ده کوکی رو با \\n لیترال (بک‌اسلش-ان) پیست کنی
    و اینجا به newline واقعی تبدیلش می‌کنیم. اگه از قبل newline واقعی
    داشته باشه (مثلاً از Raw Editor)، فقط \\n لیترال باقی‌مونده پاک می‌شه.
    """
    if not raw:
        return raw
    text = raw.strip()
...
# ─── Pro Social API (RapidAPI) - روش اصلی جدید برای پست/ریلز/استوری اینستاگرام ───
PRO_SOCIAL_HOST = "pro-social.p.rapidapi.com"

def _extract_first_video_url(d):
    """جستجوی بازگشتی برای پیدا کردن اولین URL ویدیویی داخل جواب JSON، با هر ساختاری"""
    if isinstance(d, str):
        if ".mp4" in d or "video" in d.lower() and d.startswith("http"):
            return d
        return None
    if isinstance(d, dict):
        # اول اسم‌های رایج رو مستقیم چک کن
...
def pro_social_get_userid(username: str):
    ...
    try:
        r = requests.get(
            f"https://{PRO_SOCIAL_HOST}/userinfousername/",
...
def pro_social_story_download(userid: str):
    """دانلود استوری با userid عددی، برمی‌گردونه لیستی از video_url ها"""
    try:
        r = requests.get(
            f"https://{PRO_SOCIAL_HOST}/userstorydownload/",
            headers={"x-rapidapi-key": RAPIDAPI_KEY, "x-rapidapi-host": PRO_SOCIAL_HOST},
            params={"userid": userid, "safe_url": "false"},
            timeout=25,
        )
        d = r.json()
...
def pro_social_post_detail(shortcode: str):
    ...
    try:
        ...
        d = r.json()
        logger.info(f"Pro Social post raw: {str(d)[:300]}")
        return _extract_first_video_url(d)
    except Exception as e:
...
def init_db():
    with db() as con:
        ...
        INSERT OR REPLACE INTO settings VALUES ('channel', '@InstaSaveXX');
        INSERT OR IGNORE INTO settings VALUES ('free_limit', '5');
        INSERT OR IGNORE INTO settings VALUES ('limit_enabled', '0');
        INSERT OR IGNORE INTO settings VALUES ('maintenance', '0');
        INSERT OR REPLACE INTO settings VALUES ('caption', '🎵 ربات موزیک یاب و دانلود\n @InstaSaveXX');
        INSERT OR REPLACE INTO settings VALUES ('welcome', 'سلام!\n\nلینک اینستاگرام یا پینترست بفرست، اسم آهنگ بنویس، یا از منوی زیر عکس بساز 🎨');
        ...
        CREATE TABLE IF NOT EXISTS config_invites (
            user_id       INTEGER PRIMARY KEY,
            invite_count  INTEGER DEFAULT 0,
            config_uses   INTEGER DEFAULT 0,
            invited_by    INTEGER
        );
        CREATE TABLE IF NOT EXISTS config_invite_log (
            id          INTEGER PRIMARY KEY AUTOINCREMENT,
            inviter_id  INTEGER,
            invitee_id  INTEGER,
            joined_at   TEXT,
...
# ══════════════════════════════════════════════════════════════════════════
# ─── سیستم کانفیگ رایگان V2Ray ─────────────────────────────────────────
# ══════════════════════════════════════════════════════════════════════════
import httpx
import time as _time
from bs4 import BeautifulSoup

V2NODES_BASE = "https://www.v2nodes.com"
_V2_HEADERS = {"User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 Chrome/124.0.0.0 Safari/537.36"}
CONFIG_INVITES_NEEDED = 1    # تعداد دعوت برای آنلاک
CONFIG_MAX_USES = 10         # تعداد کانفیگ قبل از نیاز به دعوت جدید

# ─── توابع دیتابیس کانفیگ ──────────────────────────────────────────────

def get_config_row(user_id):
    with db() as con:
        row = con.execute("SELECT invite_count, config_uses, invited_by FROM config_invites WHERE user_id=?", (user_id,)).fetchone()
        if row:
            return {"invite_count": row[0], "config_uses": row[1], "invited_by": row[2]}
        con.execute("INSERT OR IGNORE INTO config_invites (user_id) VALUES (?)", (user_id,))
        return {"invite_count": 0, "config_uses": 0, "invited_by": None}

def record_config_invite(inviter_id, invitee_id):
    """ثبت دعوت جدید برای inviter"""
    with db() as con:
        try:
            con.execute(
                "INSERT INTO config_invite_log (inviter_id, invitee_id, joined_at) VALUES (?,?,?)",
                (inviter_id, invitee_id, datetime.now().isoformat())
            )
            con.execute(
                "INSERT OR IGNORE INTO config_invites (user_id) VALUES (?)", (inviter_id,)
            )
            con.execute(
                "UPDATE config_invites SET invite_count=invite_count+1 WHERE user_id=?", (inviter_id,)
            )
            return True
        except Exception:
            return False  # already recorded

def increment_config_use(user_id):
    with db() as con:
        con.execute("INSERT OR IGNORE INTO config_invites (user_id) VALUES (?)", (user_id,))
        con.execute("UPDATE config_invites SET config_uses=config_uses+1 WHERE user_id=?", (user_id,))

def reset_config_cycle(user_id):
    """بعد از دعوت جدید، فقط config_uses رو ریست می‌کنه"""
    with db() as con:
        con.execute(
            "UPDATE config_invites SET config_uses=0 WHERE user_id=?", (user_id,)
        )

def can_get_config(user_id):
    """True اگه کاربر می‌تونه کانفیگ بگیره"""
    row = get_config_row(user_id)
    # اگه حداقل یه دعوت داره و هنوز ۱۰ تا نگرفته
    if row["invite_count"] >= CONFIG_INVITES_NEEDED and row["config_uses"] < CONFIG_MAX_USES:
        return True, row["config_uses"]
    # سقف ۱۰ تا تموم شده
    if row["config_uses"] >= CONFIG_MAX_USES:
        return False, row["config_uses"]
    # هنوز دعوت نکرده
    return False, row["config_uses"]

# ─── scraper v2nodes ────────────────────────────────────────────────────

async def v2_fetch_servers(count=15):
    async with httpx.AsyncClient(timeout=15, headers=_V2_HEADERS) as client:
        r = await client.get(V2NODES_BASE + "/")
        r.raise_for_status()
    ...
    for a in soup.select("a[href^='/servers/']"):
        ...
        servers.append({"title": title[:60], "url": V2NODES_BASE + href})
        if len(servers) >= count:
            break
    return servers

async def v2_fetch_config(server_url):
    try:
        async with httpx.AsyncClient(timeout=15, headers=_V2_HEADERS) as client:
        ...
        return None
    soup = BeautifulSoup(r.text, "html.parser")
    pattern = re.compile(r"(vmess|vless|trojan)://[A-Za-z0-9+/=@:.?&#%-]+", re.IGNORECASE)
    for tag in soup.find_all(["code", "pre", "textarea", "input"]):
        val = tag.get("value") or tag.get_text()
...
async def v2_measure_ping(config):
    m = re.search(r"@([^:/?#\s]+):", config)
    if not m:
        if config.startswith("vmess://"):
            import base64 as _b64, json as _j
            try:
                data = _j.loads(_b64.b64decode(config[8:] + "==").decode())
        ...
        host = m.group(1)
    try:
        async with httpx.AsyncClient(timeout=5, headers=_V2_HEADERS) as client:
            t0 = _time.monotonic()
...
async def v2_find_best():
    """بهترین کانفیگ vless/trojan/vmess با کمترین پینگ"""
    try:
        servers = await v2_fetch_servers(15)
    except Exception:
        return None
    best = None
    for s in servers:
        cfg = await v2_fetch_config(s["url"])
        if not cfg:
            continue
        ping = await v2_measure_ping(cfg)
        if ping is None or ping > 800:
            continue
        clean = re.sub(r"\s*#\d+\s*$", "", s["title"]).strip()
        if best is None or ping < best[2]:
            best = (clean, cfg, ping)
    return best

# ─── ارسال کانفیگ به کاربر ─────────────────────────────────────────────

async def send_config_to_user(message, user_id):
    result = await v2_find_best()
    if not result:
        await message.reply_text("❌ الان کانفیگ مناسب پیدا نشد. چند دقیقه دیگه دوباره امتحان کن.")
        return

    clean_title, config, ping_ms = result
    ping_icon = "🟢" if ping_ms < 150 else "🟡"
    increment_config_use(user_id)
    row = get_config_row(user_id)
    uses_left = CONFIG_MAX_USES - row["config_uses"]

    msg = (
        f"⚡️ *NEW CONFIG DROPPED*\n\n"
        f"▸ {clean_title}\n"
        f"▸ Ping: {ping_ms}ms {ping_icon}\n\n"
        f"`{config}`\n\n"
        f"─────────────────\n"
    ...
    keyboard = []
    if uses_left > 0:
        keyboard.append([InlineKeyboardButton(f"⚡️ کانفیگ بعدی ({uses_left} تا مونده)", callback_data="cfg_next")])
    else:
        keyboard.append([InlineKeyboardButton("👥 دعوت دوست برای کانفیگ بیشتر", callback_data="cfg_invite")])

    await message.reply_text(msg, parse_mode="Markdown", reply_markup=InlineKeyboardMarkup(keyboard) if keyboard else None)
...
async def config_menu_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    can, uses = can_get_config(user_id)

    if can:
        await query.message.reply_text("⏳ در حال پیدا کردن بهترین کانفیگ برات...")
        await send_config_to_user(query.message, user_id)
    else:
        row = get_config_row(user_id)
        if row["invite_count"] < CONFIG_INVITES_NEEDED:
            # هنوز دعوت نکرده
            bot_username = (await context.bot.get_me()).username
            invite_link = f"https://t.me/{bot_username}?start=ref_{user_id}"
            await query.message.reply_text(
                "👥 *برای دریافت کانفیگ رایگان*\n\n"
                "کافیه *۱ نفر* رو به ربات دعوت کنی 👇\n\n"
                f"`{invite_link}`\n\n"
                "لینک بالا رو برای دوستت بفرست — وقتی استارت زد، ۱۰ تا کانفیگ آزاد می‌شه ⚡️",
                parse_mode="Markdown",
                reply_markup=InlineKeyboardMarkup([
                    [InlineKeyboardButton("📤 اشتراک‌گذاری لینک", url=f"https://t.me/share/url?url={invite_link}&text=یه+ربات+باحال+پیدا+کردم!")],
                ])
            )
        else:
            # ۱۰ تا تموم شده، دعوت جدید لازمه
            bot_username = (await context.bot.get_me()).username
            invite_link = f"https://t.me/{bot_username}?start=ref_{user_id}"
            # invite_count رو صفر کن تا منتظر دعوت جدید باشه
            with db() as con:
                con.execute("UPDATE config_invites SET config_uses=0, invite_count=0 WHERE user_id=?", (user_id,))
            await query.message.reply_text(
                "🔄 *۱۰ تا کانفیگت تموم شد!*\n\n"
                "برای ۱۰ تا کانفیگ جدید، یه نفر دیگه دعوت کن 👇\n\n"
                f"`{invite_link}`",
                parse_mode="Markdown",
                reply_markup=InlineKeyboardMarkup([
                    [InlineKeyboardButton("📤 اشتراک‌گذاری لینک", url=f"https://t.me/share/url?url={invite_link}&text=یه+ربات+باحال+پیدا+کردم!")],
...
async def cfg_next_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    user_id = query.from_user.id
    can, _ = can_get_config(user_id)
    if can:
        await query.message.reply_text("⏳ در حال پیدا کردن کانفیگ...")
        await send_config_to_user(query.message, user_id)
    else:
        await query.message.reply_text("⛔️ سقف ۱۰ تا تموم شده! از منوی کانفیگ دعوت کن.")

async def cfg_invite_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    user_id = query.from_user.id
    bot_username = (await context.bot.get_me()).username
    invite_link = f"https://t.me/{bot_username}?start=ref_{user_id}"
    reset_config_cycle(user_id)
    await query.message.reply_text(
        "👥 *لینک دعوت تو:*\n\n"
        f"`{invite_link}`\n\n"
        "وقتی ۱ نفر از این لینک استارت بزنه، ۱۰ کانفیگ جدید آزاد می‌شه ⚡️",
        parse_mode="Markdown",
        reply_markup=InlineKeyboardMarkup([
            [InlineKeyboardButton("📤 اشتراک‌گذاری", url=f"https://t.me/share/url?url={invite_link}&text=یه+ربات+باحال+پیدا+کردم!")],
        ])
    )
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ─── ورود از طریق لینک دعوت کانفیگ ────────────────────────────
    if context.args and context.args[0].startswith("ref_"):
        try:
            inviter_id = int(context.args[0].replace("ref_", "", 1))
            if inviter_id != user.id:
                recorded = record_config_invite(inviter_id, user.id)
                if recorded:
                    try:
                        invitee_name = user.first_name or user.username or "یه نفر"
                        await context.bot.send_message(
                            inviter_id,
                            f"🎉 *{invitee_name}* از لینک دعوتت وارد ربات شد!\n\n"
                            f"✅ ۱۰ کانفیگ رایگان برات آزاد شد ⚡️\n"
                            f"از منو روی *کانفیگ رایگان V2Ray* بزن 👇",
                            parse_mode="Markdown"
                        )
                    except Exception:
                        pass
    ...
    keyboard = [
        [InlineKeyboardButton("⚡️ کانفیگ رایگان V2Ray", callback_data="cfg_menu")],
...
async def shazam_detect_from_file_async(input_path):
    ...
    try:
        ...
        logger.error("shazamio نصب نیست!")
        return None
    ...
    for ss in offsets:
        tmp_audio = input_path + f"_shazam_{ss}.wav"
        try:
            ret = subprocess.run([
            ...
            if os.path.exists(tmp_audio) and os.path.getsize(tmp_audio) > 1000:
                ...
                logger.warning(f"ffmpeg failed at ss={ss}. stderr: {ret.stderr[:200] if ret else 'N/A'}")
        except Exception as e:
...
async def audd_detect_from_file_async(input_path):
    """شناسایی آهنگ با AudD — fallback بعد از Shazam"""
    try:
        import imageio_ffmpeg
        _ffmpeg = imageio_ffmpeg.get_ffmpeg_exe()
    except Exception:
    ...
    tmp_audio = input_path + "_audd.mp3"
    try:
        subprocess.run([
            _ffmpeg, "-y", "-i", input_path,
            "-t", "20",
        ...
        data = resp.json()
        logger.info(f"AudD response: {str(data)[:300]}")
        if data.get("status") == "success" and data.get("result"):
            r = data["result"]
            return {
                "title": r.get("title", ""),
                "artist": r.get("artist", ""),
                "cover": (r.get("apple_music") or {}).get("artwork", {}).get("url", "").replace("{w}x{h}bb", "500x500bb") if r.get("apple_music") else "",
            }
    except Exception as e:
        logger.warning(f"AudD detect error: {e}")
    finally:
        if os.path.exists(tmp_audio):
            try: os.remove(tmp_audio)
            except: pass
...
async def detect_song(input_path):
    """ابتدا Shazam، اگه پیدا نکرد AudD"""
    result = await shazam_detect_from_file_async(input_path)
    if result and result.get("title"):
        logger.info(f"Shazam found: {result['title']}")
        return result
    logger.info("Shazam failed, trying AudD...")
    result = await audd_detect_from_file_async(input_path)
    if result and result.get("title"):
        logger.info(f"AudD found: {result['title']}")
    return result
...
async def download_and_send(update, context, title, artist, msg):
    ...
    # روی Railway نه ffprobe داریم نه YouTube بدون block — از SoundCloud اول بگیر
    ydl_opts = {
        ...
        # هیچ postprocessor نداریم — فایل رو rename می‌کنیم به mp3
    }

    # لیست جستجوهای مرتب شده — SoundCloud اول (کمتر block می‌کنه)
    searches = [
        f"scsearch1:{title} {artist}",
        f"scsearch1:{title} {artist} official",
        f"scsearch1:{title}",
        # YouTube به عنوان fallback
    ...
    # پیدا کردن فایل دانلود شده و rename به mp3
    if not os.path.exists(mp3_path):
        found = glob.glob(f"song_{user_id}.*")
        if found:
            os.rename(found[0], mp3_path)
    if not os.path.exists(mp3_path):
        await msg.edit_text("❌ فایل آهنگ پیدا نشد."); return

    # اسم فایل رو به "title - artist.mp3" تغییر بده تا وقتی ذخیره میشه درست باشه
    safe_name = re.sub(r'[\\/*?:"<>|]', '', f"{title} - {artist}").strip()[:80]
    named_path = f"{safe_name}.mp3"
    try:
        os.rename(mp3_path, named_path)
        audio_file = named_path
    except Exception:
        audio_file = mp3_path

    # نوشتن تگ‌های ID3 — پلیر موزیک اسم و خواننده رو از اینجا می‌خونه
    try:
        ...
        logger.warning(f"ID3 tag write failed: {e}")

    caption = get_setting("caption") or CAPTION
    await update.message.reply_audio(audio=open(audio_file,"rb"), title=title, performer=artist, caption=caption)
...
async def handle_voice(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        result = await detect_song(voice_path)
        if not result:
...
async def handle_video(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        result = await detect_song(tmp_path)
        if not result:
            await msg.edit_text(
                "😔 آهنگی شناسایی نشد\n\n"
...
async def download_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    mp3_path = f"song_{user_id}.mp3"
    ydl_opts = {"format": "bestaudio[ext=mp3]/bestaudio[ext=m4a]/bestaudio[ext=opus]/bestaudio/best",
                "outtmpl": f"song_{user_id}.%(ext)s",
                "quiet": True, "noplaylist": True}
    downloaded = False
    for s in ([url] if url else []) + [
               f"scsearch1:{title} {artist}",
               f"scsearch1:{title}",
               f"ytsearch1:{title} {artist} official audio",
               f"ytsearch1:{title} {artist}"]:
        ...
        try:
            ...
            if os.path.exists(mp3_path): downloaded = True
                ...
                found = glob.glob(f"song_{user_id}.*")
                if found: downloaded = True
        except: pass
    if not downloaded:
        await msg.edit_text("❌ آهنگ پیدا نشد."); return
    if not os.path.exists(mp3_path):
        found = glob.glob(f"song_{user_id}.*")
        if found: os.rename(found[0], mp3_path)
    if not os.path.exists(mp3_path):
        await msg.edit_text("❌ فایل آهنگ پیدا نشد."); return
    caption = get_setting("caption") or CAPTION
    try:
...
async def song_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        result = await detect_song(video_path)
        ...
        ydl_opts = {"format": "bestaudio[ext=mp3]/bestaudio[ext=m4a]/bestaudio[ext=opus]/bestaudio/best",
                    ...
                    "quiet": True, "noplaylist": True}
        downloaded = False
        for s in [f"scsearch1:{title} {artist}", f"scsearch1:{title}",
                  f"ytsearch1:{title} {artist} official audio", f"ytsearch1:{title} {artist}",
                  f"ytsearch1:{title} audio"]:
            if downloaded: break
            try:
                with yt_dlp.YoutubeDL(ydl_opts) as ydl: ydl.download([s])
        ...
        audio_file = mp3_path
        caption = get_setting("caption") or CAPTION
        await query.message.reply_audio(audio=open(audio_file,"rb"), title=title, performer=artist, caption=caption)
        add_download(user_id, "music")
        await msg.delete()
    except Exception as e:
        await msg.edit_text(f"خطا: {e}")
...
async def handle_link(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if "instagram.com" in text:
        ...
        if "/stories/" in text:
            ...
            try:
                ...
                # API 0 - Pro Social API (اولویت اول - سریع‌ترین و پایدارترین)
                try:
                    story_user_m = re.search(r'instagram\.com/stories/([^/]+)/', clean_url)
                    if story_user_m:
                        story_username = story_user_m.group(1)
                        ps_userid = pro_social_get_userid(story_username)
                        if ps_userid:
                            video_url = pro_social_story_download(ps_userid)
                except Exception as e:
                    logger.warning(f"Pro Social story flow error: {e}")
                ...
                # API 1 - SaveInsta (غیرفعال شد - DNS این دامنه هیچوقت رو Railway resolve نمیشه، فقط وقت‌کشیه)
                if False:
                    try:
                        ...
                        r_token = session.get("https://saveinsta.app/", timeout=15)
                        token_m = re.search(r'name="_token"\s+value="([^"]+)"', r_token.text)
                        if token_m:
                            token = token_m.group(1)
                            r1 = session.post("https://saveinsta.app/api/ajaxSearch",
                                data={"q": clean_url, "_token": token},
                                headers={"X-Requested-With": "XMLHttpRequest", "Referer": "https://saveinsta.app/"},
                                timeout=30)
                            d1 = r1.json()
                            logger.info(f"Story API1 (saveinsta) raw: {str(d1)[:300]}")
                            if d1.get("status") == "ok":
                                html = d1.get("data", "")
                                urls = re.findall(r'href=["\']([^"\']+\.mp4[^"\']*)["\']', html)
                                if urls:
                                    video_url = urls[0]
                    except Exception as e:
                        logger.warning(f"Story API1 (saveinsta) error: {e}")

                # API 2 - SnapSave (رایگان)
                if not video_url:
                    try:
                        session2 = requests.Session()
                        session2.headers.update({"User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36"})
                        r_snap = session2.get("https://snapsave.app/", timeout=15)
                        token2_m = re.search(r'name="_token"\s+value="([^"]+)"', r_snap.text)
                        if token2_m:
                            token2 = token2_m.group(1)
                            r2 = session2.post("https://snapsave.app/action",
                                data={"url": clean_url, "_token": token2},
                                headers={"X-Requested-With": "XMLHttpRequest", "Referer": "https://snapsave.app/"},
                                timeout=30)
                            d2 = r2.json()
                            logger.info(f"Story API2 (snapsave) raw: {str(d2)[:300]}")
                            html2 = d2.get("data", "") if isinstance(d2, dict) else ""
                            urls2 = re.findall(r'href=["\']([^"\']+\.mp4[^"\']*)["\']', html2)
                            if urls2:
                                video_url = urls2[0]
                    except Exception as e:
                        logger.warning(f"Story API2 (snapsave) error: {e}")

                # API 3 - igram.world (رایگان)
                if not video_url:
                    try:
                        session3 = requests.Session()
                        session3.headers.update({"User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36"})
                        r_ig = session3.get("https://igram.world/", timeout=15)
                        token3_m = re.search(r'name="_token"\s+value="([^"]+)"', r_ig.text)
                        if not token3_m:
                            token3_m = re.search(r'"_token":"([^"]+)"', r_ig.text)
                        if token3_m:
                            token3 = token3_m.group(1)
                            r3 = session3.post("https://igram.world/api/convert",
                                json={"url": clean_url, "_token": token3},
                                headers={"X-Requested-With": "XMLHttpRequest", "Referer": "https://igram.world/", "Content-Type": "application/json"},
                                timeout=30)
                            d3 = r3.json()
                            logger.info(f"Story API3 (igram) raw: {str(d3)[:300]}")
                            if isinstance(d3, dict):
                                items = d3.get("url", []) or d3.get("data", [])
                                if isinstance(items, list):
                                    for item in items:
                                        if isinstance(item, dict) and "mp4" in item.get("url", ""):
                                            video_url = item["url"]
                                            break
                                elif isinstance(items, str) and "mp4" in items:
                                    video_url = items
                    except Exception as e:
                        logger.warning(f"Story API3 (igram) error: {e}")

                # API 4 - instagram-scraper-api2 با RapidAPI
                if not video_url:
                    try:
                        import re as _re2
                ...
                # API 5 - yt-dlp با کوکی اینستاگرام
                if not video_url:
                    try:
                        ig_cookies = (os.environ.get("INSTAGRAM_COOKIES", "") or os.environ.get("IG_COOKIES", ""))
                        ...
                        if ig_cookies:
                            ...
                            with open(cookie_file_path, "w") as cf:
                                cf.write(normalize_cookie_text(ig_cookies))
                        elif ig_cookies_file and os.path.exists(ig_cookies_file):
                            cookie_file_path = ig_cookies_file
                        logger.info(f"Story cookie file: {cookie_file_path}")
            ...
            async def _send_instagram_video(path):
                """ارسال ویدیو اینستاگرام + شناسایی آهنگ"""
                channel = get_setting("channel")
                keyboard = [[InlineKeyboardButton("کانال ما 📢", url=f"https://t.me/{channel.lstrip('@')}")]]
                ...
                try:
                    ...
                    logger.warning(f"Shazam error on instagram: {se}")
                    await msg.edit_text("🎵 آهنگی شناسایی نشد 😔\n\nاگه اسم آهنگ رو میدونی مستقیم بنویس 👇")

            downloaded = False

            # ─── روش ۰: Pro Social API (اولویت اول - سریع‌ترین و پایدارترین) ───
            try:
                import re as _re_sc
                sc_m = _re_sc.search(r'instagram\.com/(?:p|reel|reels|tv)/([A-Za-z0-9_-]+)', clean_url)
                if sc_m:
                    shortcode = sc_m.group(1)
                    ps_video_url = pro_social_post_detail(shortcode)
                    if ps_video_url:
                        video_data = requests.get(ps_video_url, timeout=45).content
                        with open(video_path, "wb") as f:
                            f.write(video_data)
                        await _send_instagram_video(video_path)
                        downloaded = True
            except Exception as e:
                logger.warning(f"Pro Social post flow error: {e}")

            # ─── روش ۱: RapidAPI (سریع‌تر و مطمئن‌تر) ───────────────────────
            rapidapi_hosts = [
            ...
            video_url = None
            for host in (rapidapi_hosts if not downloaded else []):
                if video_url:
                    break
                try:
                    ...
                    if r_api.status_code != 200:
                        continue
                    d = r_api.json()
                    logger.info(f"Instagram API ({host}) raw: {str(d)[:300]}")
                    # استخراج ویدیو URL از پاسخ‌های مختلف
                    if isinstance(d, dict):
                    ...
                    logger.info(f"Instagram API video_url: {video_url}")
                except Exception as e:
                    logger.warning(f"Instagram RapidAPI ({host}) error: {e}")

            if video_url:
                try:
                    video_data = requests.get(video_url, timeout=45).content
                    with open(video_path, "wb") as f:
                        f.write(video_data)
                    await _send_instagram_video(video_path)
                    downloaded = True
            ...
            if not downloaded:
                try:
                    # اول مدت ویدیو رو بفهم تا کیفیت مناسب انتخاب بشه
                    try:
                        with yt_dlp.YoutubeDL({"quiet": True, "skip_download": True, "socket_timeout": 15}) as ydl_info:
                            info = ydl_info.extract_info(text, download=False)
                            vid_duration = info.get("duration", 0) or 0
                    except Exception:
                        vid_duration = 0

                    if vid_duration > 60:
                        quality_fmt = "best[height<=480][ext=mp4]/best[height<=480]/best"
                    elif vid_duration > 30:
                        quality_fmt = "best[height<=720][ext=mp4]/best[height<=720]/best"
                    else:
                        quality_fmt = "best[height<=1080][ext=mp4]/best[height<=1080]/best"
                    ...
                    # اگر کوکی اینستاگرام موجود است، استفاده کن
            ...
            if not downloaded:
                await msg.edit_text("❌ دانلود ممکن نشد.\n\nممکنه پست خصوصی باشه یا لینک اشتباه باشه.")
    ...
    # ─── پینترست ──────────────────────────────────────────────
        ...
        try:
            ...
            # روش ۱: scrape صفحه پینترست - ویدیو
            try:
                ...
                video_patterns = [
                    ...
                    r'"V_480p"[^}]*"url":"([^"]+)"',
                    r'"V_360p"[^}]*"url":"([^"]+)"',
                    r'"video_list":\{[^}]*"V_[^"]*":\{"width":\d+,"height":\d+,"duration":\d+,"url":"([^"]+)"',
                ...
                for pat in video_patterns:
                    ...
                    if m:
                        ...
                        is_video = True
                        logger.info(f"Pinterest video found via scrape: {media_url[:80]}")
                        break
            except Exception as e:
                logger.warning(f"Pinterest scrape video error: {e}")

            # روش ۲: scrape صفحه پینترست - عکس
            if not media_url:
                try:
                    page = requests.get(pin_url, headers=headers_scrape, timeout=20)
                    html = page.text
                    ...
                    for pat in img_patterns:
                        ...
                        if m:
                            media_url = m.group(1).replace("\\u002F", "/").replace("\\/", "/")
                            is_video = False
                            logger.info(f"Pinterest image found via scrape: {media_url[:80]}")
                            break
                except Exception as e:
                    logger.warning(f"Pinterest scrape image error: {e}")

            # روش ۳: yt-dlp با تنظیمات بهتر
            if not media_url:
                try:
                    ydl_opts = {
                        "outtmpl": f"pinterest_{user.id}.%(ext)s",
                    ...
                    if matches:
                        ...
                        if ext in ("mp4", "mov", "webm"):
                            ...
                            await update.message.reply_photo(photo=open(file_path, "rb"), caption=caption,
                                reply_markup=InlineKeyboardMarkup(keyboard))
                        sent = True
                        logger.info(f"Pinterest yt-dlp success: {file_path}")
                except Exception as e:
                    logger.warning(f"Pinterest yt-dlp error: {e}")

            # دانلود و ارسال media_url
            if not sent and media_url:
                try:
                    # تبدیل HEIC و فرمت‌های ناسازگار به JPG
                    clean_media_url = media_url.split("?")[0]
                    ext_out = clean_media_url.rsplit(".", 1)[-1].lower() if "." in clean_media_url else ""
                    if ext_out in ("heic", "heif", "avif", "webp") or not ext_out:
                        # جایگزینی URL با نسخه JPG پینترست
                        media_url_jpg = re.sub(r'/originals/', '/736x/', media_url)
                        media_url_jpg = re.sub(r'\.(heic|heif|avif|webp)(\?.*)?$', '.jpg', media_url_jpg)
                        logger.info(f"Pinterest converting to JPG: {media_url_jpg[:80]}")
                        try:
                            dl_content = requests.get(media_url_jpg, headers=headers_scrape, timeout=40).content
                            ext_out = "jpg"
                        except:
                            dl_content = requests.get(media_url, headers=headers_scrape, timeout=40).content
                    else:
                        dl_content = requests.get(media_url, headers=headers_scrape, timeout=40).content
                    if not ext_out or ext_out in ("heic", "heif", "avif"):
                        ext_out = "jpg"
                    file_path = f"pinterest_{user.id}.{ext_out}"
                    with open(file_path, "wb") as fp:
                        fp.write(dl_content)
                    caption = get_setting("caption") or CAPTION
                    if is_video or ext_out in ("mp4", "mov", "webm"):
                        await update.message.reply_video(video=open(file_path, "rb"), caption=caption,
                            reply_markup=InlineKeyboardMarkup(keyboard), supports_streaming=True)
                    else:
                        await update.message.reply_photo(photo=open(file_path, "rb"), caption=caption,
                    ...
                    logger.warning(f"Pinterest download media_url error: {e}")
            ...
            logger.warning(f"Pinterest main error: {e}")
            await msg.edit_text(f"خطا در دانلود پینترست: {e}")
        finally:
            for f in glob.glob(f"pinterest_{user.id}.*"):
                try: os.remove(f)
                except: pass
...
def admin_main_keyboard():
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("🔧 حالت تعمیر", callback_data="adm_maintenance"),
         InlineKeyboardButton("🔢 محدودیت دانلود", callback_data="adm_toggle_limit")],
    ])
...
async def admin_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "adm_back":
        ...
        st = "🟡 روشن" if new_val == "1" else "🟢 خاموش"
        limit_st2 = "🟡 روشن" if get_setting("limit_enabled") == "1" else "🟢 خاموش"
        maintenance2 = "🟡 روشن" if get_setting("maintenance") == "1" else "🟢 خاموش"
        await query.message.edit_text(f"🔧 *پنل ادمین*\n\nحالت تعمیر: {maintenance2}\nمحدودیت دانلود: {st}",
            parse_mode="Markdown", reply_markup=admin_main_keyboard())
...
app.add_handler(CallbackQueryHandler(config_menu_callback, pattern="^cfg_menu$"))
app.add_handler(CallbackQueryHandler(cfg_next_callback, pattern="^cfg_next$"))
app.add_handler(CallbackQueryHandler(cfg_invite_callback, pattern="^cfg_invite$"))
