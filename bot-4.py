...
async def handle_link(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if "instagram.com" in text:
        ...
        try:
            ...
            r = requests.get(
                "https://youtube-mp310.p.rapidapi.com/info",
                headers=yt_headers,
                params={"url": text},
                timeout=30
            )
...
async def yt_quality_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_audio:
        try:
            r = requests.get(
                "https://youtube-mp310.p.rapidapi.com/mp3",
                headers=yt_headers,
                params={"url": url},
                timeout=60
            )
            ...
            if not download_url:
                await msg.edit_text(f"❌ خطا: {data_r}")
                return
            keyboard = [[InlineKeyboardButton("کانال ما 📢", url=f"https://t.me/{clean_ch}")]]
            ...
            await msg.edit_text(f"❌ دانلود صدا ممکن نشد: {e}")
    else:
        height = data.replace("yt_video_", "")
        try:
            r = requests.get(
                "https://youtube-mp310.p.rapidapi.com/mp4",
                headers=yt_headers,
            ...
            await msg.edit_text(f"❌ دانلود ویدیو ممکن نشد: {e}")
