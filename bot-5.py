...
async def yt_quality_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    apis = [
        {"host": "youtube-mp310.p.rapidapi.com", "mp3": "https://youtube-mp310.p.rapidapi.com/mp3", "mp4": "https://youtube-mp310.p.rapidapi.com/mp4"},
        {"host": "youtube-mp36.p.rapidapi.com", "mp3": "https://youtube-mp36.p.rapidapi.com/mp3", "mp4": "https://youtube-mp36.p.rapidapi.com/mp4"},
    ]
    ...
    if is_audio:
        ...
        for api in apis:
            try:
                yt_headers = {"x-rapidapi-key": RAPIDAPI_KEY, "x-rapidapi-host": api["host"]}
                r = requests.get(api["mp3"], headers=yt_headers, params={"url": url}, timeout=60)
                data_r = r.json()
        ...
        for api in apis:
            try:
                yt_headers = {"x-rapidapi-key": RAPIDAPI_KEY, "x-rapidapi-host": api["host"]}
                r = requests.get(api["mp4"], headers=yt_headers, params={"url": url, "quality": f"{height}p"}, timeout=60)
                data_r = r.json()
                download_url = data_r.get("url") or data_r.get("download_url") or data_r.get("link")
                if download_url:
                    break
