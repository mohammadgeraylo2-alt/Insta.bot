...
async def handle_link(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if "instagram.com" in text:
        ...
        try:
            ...
            if not sent:
                host = "pinterest-downloader-download-pinterest-videos-images.p.rapidapi.com"
                headers = {
                    "x-rapidapi-key": RAPIDAPI_KEY,
                    "x-rapidapi-host": host
                }
                r = requests.get(
                    f"https://{host}/pinterest-url",
                    headers=headers,
                    params={"url": text},
                    timeout=30
                )
                ...
                if isinstance(data, dict):
                    ...
                    if not media_url and data.get("data"):
                        d = data["data"]
                        if isinstance(d, list) and d:
                            media_url = d[0].get("url")
                ...
                if not media_url:
                    await msg.edit_text("محتوا پیدا نشد، لینک رو چک کن.")
                    return
