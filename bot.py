...
async def handle_link(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if "instagram.com" in text:
        ...
        if "/stories/" in text:
            ...
            def _download_video_url(url, dest_path):
                """دانلود stream-based با اعتبارسنجی content-type و حجم فایل"""
                    ...
                    with open(dest_path, "wb") as f:
                        for chunk in r.iter_content(chunk_size=1024 * 256):
                            if chunk:
                                f.write(chunk)
                                total += len(chunk)
                    if total < 10_000:  # کمتر از ۱۰کیلوبایت یعنی احتمالاً فایل معتبر نیست
                        ...
                        if media:
                            for m in media:
                                if isinstance(m, dict) and (m.get("type", "") == "video" or m.get("video_url")):
                                    video_url = m.get("video_url") or m.get("url") or m.get("link")
                                    break
                            if not video_url and media and isinstance(media[0], dict):
                                video_url = media[0].get("url") or media[0].get("video_url") or media[0].get("link")
                        if not video_url:
                            video_url = (
                                d.get("url") or d.get("video_url") or d.get("download_url")
                                or d.get("link") or d.get("video")
                            )
                        if not video_url and isinstance(d.get("data"), dict):
                            data_d = d["data"]
                            video_url = data_d.get("url") or data_d.get("video_url") or data_d.get("download_url")
                    elif isinstance(d, list) and d and isinstance(d[0], dict):
                        video_url = d[0].get("url") or d[0].get("video_url") or d[0].get("link")
                ...
                try:
                    ...
                    if token_m:
                        ...
                        r1 = session.post("https://saveinsta.app/api/ajaxSearch",
                            ...
                            timeout=30)
                        d1 = r1.json()
                        logger.info(f"Instagram fallback (saveinsta) raw: {str(d1)[:300]}")
                        if d1.get("status") == "ok":
                            html = d1.get("data", "")
                            urls = re.findall(r'href=["\']([^"\']+\.mp4[^"\']*)["\']', html)
                            if urls:
                                fallback_url = urls[0]
                    if fallback_url:
                        _download_video_url(fallback_url, video_path)
                        await _send_instagram_video(video_path)
                        downloaded = True
                except Exception as e:
                    logger.warning(f"Instagram SaveInsta fallback error: {e}")
                    if os.path.exists(video_path):
                        try: os.remove(video_path)
                        except: pass

            # ─── روش ۳: SnapSave (رایگان) ────────────────────────────────────
            if not downloaded:
                try:
                    session2 = requests.Session()
                    session2.headers.update({"User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36"})
                    ...
                    if token2_m:
                        ...
                        r2 = session2.post("https://snapsave.app/action",
                            ...
                            timeout=30)
                        d2 = r2.json()
                        logger.info(f"Instagram fallback (snapsave) raw: {str(d2)[:300]}")
                        html2 = d2.get("data", "") if isinstance(d2, dict) else ""
                        urls2 = re.findall(r'href=["\']([^"\']+\.mp4[^"\']*)["\']', html2)
                        if urls2:
                            fallback_url2 = urls2[0]
                    if fallback_url2:
                        _download_video_url(fallback_url2, video_path)
                        await _send_instagram_video(video_path)
                        downloaded = True
                except Exception as e:
                    logger.warning(f"Instagram SnapSave fallback error: {e}")
                    if os.path.exists(video_path):
                        try: os.remove(video_path)
                        except: pass

            # ─── روش ۴: yt-dlp (fallback نهایی) ──────────────────────────────
            if not downloaded:
                try:
                    ...
                    try:
                        with yt_dlp.YoutubeDL({"quiet": True, "skip_download": True, "socket_timeout": 15}) as ydl_info:
                            info = ydl_info.extract_info(clean_url, download=False)
                    ...
                    # اگر کوکی اینستاگرام موجود است، استفاده کن (چند اسم متغیر پشتیبانی می‌شه)
                    ig_cookies = INSTAGRAM_COOKIES or os.environ.get("IG_COOKIES", "")
                    ig_cookies_file = os.environ.get("IG_COOKIES_FILE", "")
