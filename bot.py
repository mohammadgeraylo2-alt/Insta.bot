...
async def yt_quality_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_audio:
        try:
            ...
            # گرفتن لینک دانلود از yt-api
            r = requests.get("https://yt-api.p.rapidapi.com/dl",
                             headers=headers, params={"id": url}, timeout=60)
            d = r.json()
            raise Exception(f"DEBUG: status={r.status_code} keys={list(d.keys())} data={str(d)[:300]}")
            dl_url = None
            # پیدا کردن بهترین فرمت صوتی
            formats = d.get("formats", [])
            # ساختارهای مختلف API
            if not formats:
                formats = d.get("adaptiveFormats", [])
            if not formats and d.get("url"):
                dl_url = d.get("url")
            if not dl_url:
                for fmt in formats:
        ...
        try:
            ...
            # اول دنبال فرمت ترکیبی (ویدیو+صدا) با کیفیت انتخابی بگرد
            for fmt in all_formats:
                q = str(fmt.get("qualityLabel", ""))
                has_video = fmt.get("hasVideo", False)
                has_audio = fmt.get("hasAudio", False)
                if has_video and has_audio and str(height) in q:
                    video_url = fmt.get("url")
                    break

            # اگه پیدا نشد، ویدیو و صدا رو جدا پیدا کن
            if not video_url:
                for fmt in all_formats:
                    if fmt.get("hasVideo") and str(height) in str(fmt.get("qualityLabel", "")):
                        video_url = fmt.get("url")
                        break
                if not video_url:
                    for fmt in all_formats:
            ...
            if not video_url:
                raise Exception("فرمت ویدیو پیدا نشد")
