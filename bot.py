...
async def song_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        # اول از URL اینستاگرام که ذخیره شده استفاده میکنیم
        insta_url = user_urls.get(user_id)
        host = "reels-tiktok-shorts-song-recognition-api-shazam.p.rapidapi.com"
        headers = {"x-rapidapi-key": RAPIDAPI_KEY, "x-rapidapi-host": host}
        if insta_url:
            r = requests.get(f"https://{host}/recognize/social/url", headers=headers, params={"url": insta_url})
        else:
            # اگه URL نبود سعی میکنیم از فایل ویدیو بخونیم
