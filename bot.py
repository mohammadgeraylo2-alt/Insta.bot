...
async def handle_voice(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        resp_json = shazam_detect_from_file(f"voice_{user.id}.ogg", RAPIDAPI_KEY)
...
async def handle_video(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # فایل رو دانلود میکنیم و به صورت base64 به Shazam میدیم
...
async def download_and_send(update, context, title, artist, msg):
    ...
    # اگه هیچکدام کار نکرد، فایل با پسوند دیگه رو mp3 کن
    if not downloaded:
...
# ─── شناسایی آهنگ با Shazam از روی فایل صوتی (base64) ───────
async def shazam_detect_from_file(input_path, rapidapi_key):
    """صدا رو extract میکنه و به Shazam میده"""
    import subprocess
    ...
    # با ffmpeg فقط ۱۵ ثانیه صدا رو با کیفیت پایین extract میکنیم
    tmp_audio = input_path + "_shazam.ogg"
    try:
        subprocess.run([
            "ffmpeg", "-y", "-i", input_path,
            "-t", "15",          # فقط ۱۵ ثانیه
            "-vn",               # بدون ویدیو
            "-ar", "22050",      # sample rate پایین
            "-ac", "1",          # mono
            "-q:a", "5",
            tmp_audio
    ...
    r = requests.post(f"https://{host}/songs/v2/detect", headers=headers, data=audio_b64, timeout=30)
    return r.json()
...
async def song_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    audio_path = f"detect_{user_id}.ogg"
    try:
        # ۱. ویدیو رو از اینستاگرام دوباره دانلود میکنیم (فقط ۱۵ ثانیه اول)
        insta_url = user_urls.get(user_id)
        if not insta_url:
            await msg.edit_text("لینک اینستاگرام پیدا نشد، دوباره لینک رو بفرست."); return

        ydl_opts = {
            "outtmpl": f"detect_{user_id}.%(ext)s",
            "format": "worstaudio/worst",  # کمترین حجم
            "noplaylist": True,
            "quiet": True,
            "postprocessors": [{"key": "FFmpegExtractAudio", "preferredcodec": "vorbis"}],
            "postprocessor_args": ["-t", "15"],  # فقط ۱۵ ثانیه
        }
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            ydl.download([insta_url])

        # فایل صوتی که ساخته شده رو پیدا کن
        found = glob.glob(f"detect_{user_id}.*")
        if not found:
            await msg.edit_text("دانلود ویدیو برای شناسایی آهنگ ناموفق بود."); return
        audio_path = found[0]

        # ۲. فایل رو به Shazam بده
        resp_json = shazam_detect_from_file(audio_path, RAPIDAPI_KEY)
        track = resp_json.get("track")
        if not track:
            # نشون بده API چی برگردوند تا بفهمیم مشکل کجاست
            raw = str(resp_json)[:300]
        ...
        if not downloaded:
            ...
            if found:
                ...
                downloaded = True
        if not downloaded:
            await msg.edit_text(f"❌ دانلود آهنگ ناموفق بود.\n\n🎵 آهنگ شناسایی شد: {title} - {artist}\n\n⚠️ خطا: {last_error[:200]}")
            return
        caption = get_setting("caption") or CAPTION
        await query.message.reply_audio(audio=open(mp3_path,"rb"), title=title, performer=artist, caption=caption)
        add_download(user_id, "music")
        ...
        for f in glob.glob(f"detect_{user_id}.*"):
            try: os.remove(f)
            except: pass
        for f in glob.glob(f"song_{user_id}.*"):
            try: os.remove(f)
