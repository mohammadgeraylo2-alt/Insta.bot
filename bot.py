...
def can_get_config(user_id):
    ...
    if row["invite_count"] < CONFIG_INVITES_NEEDED and row["config_uses"] == 0:
        return False, 0  # هنوز دعوت نکرده
    if row["config_uses"] >= CONFIG_MAX_USES:
        return False, row["config_uses"]  # سقف ۱۰ تا تموم شده
    ...
    if context.args and context.args[0].startswith("ref_"):
        try:
            ...
            if inviter_id != user.id:
                ...
                if recorded:
                    try:
                        row = get_config_row(inviter_id)
                        await context.bot.send_message(
                            inviter_id,
                            f"🎉 یه نفر از لینک دعوتت استارت زد!\n"
