...
async def yt_quality_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_audio:
        ...
        try:
            file_path = f"yt_{user_id}.mp4"
            headers = {"x-rapidapi-key": RAPIDAPI_KEY, "x-rapidapi-host": "yt-api.p.rapidapi.com"}

            # استخراج ID از URL یوتیوب
            yt_id = url
            ...
            adaptive = d.get("adaptiveFormats", [])
            all_formats = formats + adaptive

            # اول دنبال فرمت با کیفیت انتخابی
            for fmt in all_formats:
                q = str(fmt.get("qualityLabel", "") or fmt.get("quality", ""))
                mime = str(fmt.get("mimeType", "") or fmt.get("type", ""))
                url_f = fmt.get("url")
                if url_f and str(height) in q and "video" in mime:
                    video_url = url_f
                    break

            # اگه پیدا نشد، هر فرمت ویدیویی
            if not video_url:
                for fmt in all_formats:
                    q = str(fmt.get("qualityLabel", "") or fmt.get("quality", ""))
                    mime = str(fmt.get("mimeType", "") or fmt.get("type", ""))
                    url_f = fmt.get("url")
                    if url_f and "video" in mime:
                        video_url = url_f
                        if str(height) in q:
                            break
            ...
            for fmt in all_formats:
                mime = str(fmt.get("mimeType", "") or fmt.get("type", ""))
                url_f = fmt.get("url")
                if url_f and "audio" in mime and "video" not in mime:
                    audio_url = url_f
                    break

            if not video_url:
                raise Exception(f"فرمت ویدیو پیدا نشد - {d.keys()}")

            import subprocess
            v_path = f"yt_v_{user_id}.tmp"
            ...
            if audio_url:
                a_path = f"yt_a_{user_id}.tmp"
                r3 = requests.get(audio_url, timeout=300, stream=True)
                ...
                if not os.path.exists(file_path) or os.path.getsize(file_path) == 0:
                    # اگه merge نشد، فقط ویدیو رو بفرست
                    os.rename(v_path, file_path)
