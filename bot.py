...
async def shazam_detect_from_file_async(input_path):
    ...
    tmp_audio = input_path + "_shazam.wav"

    try:
        # با ffmpeg صدا رو استخراج کن (wav — shazamio بهتر میخونه)
        ret = subprocess.run([
        ...
        if os.path.exists(tmp_audio) and os.path.getsize(tmp_audio) > 1000:
            ...
            logger.warning(f"ffmpeg wav extraction failed. stderr: {ret.stderr[:200] if ret else 'N/A'}")
