...
def enhance_prompt(text):
    if not GEMINI_KEY:
        return text + ", ultra detailed, 8k, photorealistic, cinematic lighting"
    try:
        msg = (
            "You are an expert AI image prompt engineer specializing in photorealistic images. "
            "The user gives a simple idea possibly in Persian. "
            "1) Translate to English if needed. "
            "2) Expand into a rich PHOTOREALISTIC prompt. Always include: "
            "RAW photo, DSLR, photorealistic, hyperrealistic, ultra detailed, sharp focus, "
            "real skin texture, natural lighting or cinematic lighting, 8k resolution. "
            "NEVER use words like: cartoon, anime, illustration, painting, drawing, artistic, render. "
            "Return ONLY the final English prompt, nothing else. Max 150 words. "
            "User idea: " + text
        )
        payload = {
...
async def modelslab_generate(prompt, negative_prompt=""):
    ...
    try:
        payload = {
            ...
            "model_id": "flux",
            "prompt": prompt,
            "negative_prompt": negative_prompt or "blurry, low quality, cartoon, anime, drawing, nsfw",
            "width": "1024",
            "height": "1024",
            "samples": "1",
        ...
        if r.status_code != 200:
            logger.warning(f"ModelsLab text2img HTTP {r.status_code}: {r.text[:200]}")
            return None
        data = r.json()
        ...
        # اگه processing بود، polling انجام بده
        elif status == "processing":
            fetch_url = data.get("future_links", [None])[0] or data.get("fetch_result")
            fetch_key = data.get("id")
            for _ in range(15):
                await asyncio.sleep(5)
                if fetch_url:
                    poll = requests.get(fetch_url, timeout=30)
                elif fetch_key:
                    poll = requests.post("https://modelslab.com/api/v6/realtime/fetch",
        ...
        logger.warning(f"ModelsLab generate error: {e}")
    return None
...
async def modelslab_edit(img_bytes, instruction, negative_prompt=""):
    ...
    try:
        ...
        payload = {
            ...
            "model_id": "flux",
            "prompt": instruction,
            "negative_prompt": negative_prompt or "blurry, low quality, nsfw",
...
# ─── ادیت عکس: ModelsLab → Gemini fallback ───────────────────────────────
async def ai_edit_image(img_bytes, instruction):
    import asyncio

    # ۱) اول ModelsLab img2img امتحان کن
    if MODELSLAB_KEY:
        # ترجمه/بهبود instruction با Gemini اگه موجوده
    ...
    try:
        if not GEMINI_KEY:
            return None, "کلید MODELSLAB_KEY یا GEMINI_KEY تنظیم نشده."
        pil = Image.open(io.BytesIO(img_bytes)).convert("RGB")
        buf = io.BytesIO()
        pil.save(buf, format="PNG")
        img_b64 = base64.b64encode(buf.getvalue()).decode()
        ...
        return None, "عکسی در پاسخ پیدا نشد."
    except Exception as e:
        return None, str(e)
