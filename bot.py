...
async def yt_quality_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_audio:
        ...
        try:
            ...
            mp3_apis = [
                {"host": "youtube-mp310.p.rapidapi.com", "url": "https://youtube-mp310.p.rapidapi.com/download"},
                {"host": "youtube-mp36.p.rapidapi.com", "url": "https://youtube-mp36.p.rapidapi.com/download"},
            ]
            video_url = None
            last_err = None
            for api in mp3_apis:
                try:
                    ...
                    print("Response:", d)
                    video_url = d.get("url") or d.get("link") or d.get("download_url") or d.get("downloadUrl") or d.get("videoUrl")
                    if video_url:
                        break
                    last_err = d
                except Exception as e:
            ...
            # اول لینک رو بفرست تا ببینیم کار میکنه
            await query.message.reply_text(f"🔗 لینک ویدیو: {video_url}")
            try:
                ...
                print("reply_video error:", video_err)
                await msg.edit_text(f"❌ تلگرام لینک رو قبول نکرد: {video_err}")
        except Exception as e:
            import traceback
