...
async def ai_generate_image(prompt):
    ...
    # ۲) HuggingFace FLUX.1-dev — کیفیت خیلی بالا
    ...
    # ۳) آخر Pollinations به عنوان backup
    try:
        ...
        if r.status_code == 200 and r.headers.get("content-type", "").startswith("image"):
            return r.content, None
        return None, f"هر دو سرویس خطا دادن (HTTP {r.status_code})"
