...
# ─── چت با هوش مصنوعی (Gemini رایگان) ────────────────────────────────────
user_chat_history = {}  # تاریخچه مکالمه هر کاربر

async def ask_claude(user_id: int, user_message: str) -> str:
    if not GEMINI_KEY:
        return "❌ کلید GEMINI_KEY تنظیم نشده. لطفاً با ادمین تماس بگیر."
    history = user_chat_history.get(user_id, [])
    history.append({"role": "user", "parts": [{"text": user_message}]})
    history = history[-20:]
    try:
        system_instruction = "تو یه دستیار هوشمند فارسی‌زبان هستی. کوتاه، مفید و دوستانه جواب بده. اگه سوال فنی یا انگلیسی بود، به فارسی توضیح بده."
        payload = {
        ...
        if r.status_code != 200:
            return f"❌ خطا از Gemini: {r.status_code}"
        data = r.json()
        if "error" in data:
            return f"❌ خطا: {data['error'].get('message', 'نامشخص')}"
