    ...
    if "instagram.com" in text:
        ...
        if "/stories/" in text:
            ...
            try:
                ...
                # API 1
                try:
                    host1 = "instagram-downloader-download-instagram-stories-videos4.p.rapidapi.com"
                    r1 = requests.get(f"https://{host1}/convert",
                        headers={"x-rapidapi-key": RAPIDAPI_KEY, "x-rapidapi-host": host1},
                        params={"url": text}, timeout=30)
                    d1 = r1.json()
                    ...
                    logger.info(f"Story API1 result: video_url={video_url}, raw={str(d1)[:200]}")
                except Exception as e:
                ...
                if not video_url:
                    try:
                        host2 = "instagram-looter2.p.rapidapi.com"
                        r2 = requests.get(f"https://{host2}/story",
                            headers={"x-rapidapi-key": RAPIDAPI_KEY, "x-rapidapi-host": host2},
                            params={"url": text}, timeout=30)
                        d2 = r2.json()
                        if isinstance(d2, list) and d2:
                            video_url = d2[0].get("video_url") or d2[0].get("url")
                        elif isinstance(d2, dict):
                            video_url = d2.get("video_url") or d2.get("url")
                        logger.info(f"Story API2 result: video_url={video_url}, raw={str(d2)[:200]}")
                    except Exception as e:
                        logger.warning(f"Story API2 error: {e}")

                # API 3 - instagram-reels-media-downloader
                if not video_url:
                    try:
                        host3 = "instagram-reels-media-downloader.p.rapidapi.com"
                        r3 = requests.get(f"https://{host3}/download",
                            headers={"x-rapidapi-key": RAPIDAPI_KEY, "x-rapidapi-host": host3},
                            params={"url": text}, timeout=30)
                        d3 = r3.json()
                        if isinstance(d3, dict):
                            video_url = d3.get("video_url") or d3.get("url") or d3.get("download_url")
                            if not video_url:
                                media = d3.get("media", []) or d3.get("medias", [])
                                if media:
                                    video_url = media[0].get("url") or media[0].get("video_url")
                        elif isinstance(d3, list) and d3:
                            video_url = d3[0].get("video_url") or d3[0].get("url")
                        logger.info(f"Story API3 result: video_url={video_url}, raw={str(d3)[:200]}")
                    except Exception as e:
                        logger.warning(f"Story API3 error: {e}")

                # API 4 - yt-dlp (مستقیم دانلود، بدون نیاز به video_url)
                if not video_url:
                    try:
