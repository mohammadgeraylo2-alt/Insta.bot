...
# توکن رایگان از https://dashboard.audd.io بگیر (اختیاری — برای شناسایی آهنگ‌های کمیاب/زیرزمینی به‌عنوان لایه دوم)
...
# ─── لایه دوم شناسایی آهنگ (AudD) ───────────────────────────
# Shazam روی آهنگ‌های کمیاب/زیرزمینی/ریمیکس خیلی وقتا جواب نمیده.
# AudD یه دیتابیس جدا داره و معمولاً این جور آهنگ‌ها رو بهتر تشخیص میده.
# اگه AUDD_API_TOKEN ست نشده باشه، این لایه به‌سادگی نادیده گرفته میشه.
def recognize_audd_url(file_url):
    ...
    try:
        ...
        if data.get("status") == "success" and result:
            return {"title": result.get("title", "نامشخص"), "subtitle": result.get("artist", "نامشخص")}
    except Exception as e:
        logger.warning(f"[AudD-url] خطا: {e}")
    return None

def recognize_audd_file(file_path):
    if not AUDD_API_TOKEN:
        return None
    try:
        ...
        logger.warning(f"[AudD-file] خطا: {e}")
    return None

# ─── پیدا کردن بهترین لینک برای دانلود (مخصوص آهنگ‌های کمیاب) ─
# به‌جای دانلود اولین نتیجه‌ی هر سرچ، چند تا نتیجه از ساندکلاود و یوتیوب
# می‌گیریم و با fuzzy_score بهترین تطبیق رو انتخاب می‌کنیم.
def find_download_url(title, artist):
    query = f"{title} {artist}".strip()
    if not query:
        return None
    candidates = []
    for prefix, count in (("scsearch", 5), ("ytsearch", 5)):
        try:
            with yt_dlp.YoutubeDL({"quiet": True, "extract_flat": True, "noplaylist": True}) as ydl:
                ...
                for entry in result.get("entries", []) or []:
                    ...
                    if not url:
                        continue
                    candidates.append({"url": url, "score": fuzzy_score(query, entry_title)})
        except Exception as e:
            logger.warning(f"[find_download_url] {prefix} خطا: {e}")
    if not candidates:
        return None
...
async def handle_voice(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if not track:
            # لایه دوم: AudD (برای آهنگ‌های کمیاب/زیرزمینی)
            track_fallback = recognize_audd_file(voice_path)
            if not track_fallback:
                await msg.edit_text("آهنگی شناسایی نشد"); return
            title, artist = track_fallback["title"], track_fallback["subtitle"]
        else:
...
async def handle_video(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if not track:
            # لایه دوم: AudD (برای آهنگ‌های کمیاب/زیرزمینی)
            track_fallback = recognize_audd_url(file.file_path)
            if not track_fallback:
                await msg.edit_text("آهنگی شناسایی نشد"); return
            title, artist = track_fallback["title"], track_fallback["subtitle"]
        else:
...
async def download_and_send(update, context, title, artist, msg):
    ...
    downloaded = False
    best_url = find_download_url(title, artist)
    searches = ([best_url] if best_url else []) + [f"scsearch1:{title} {artist}", f"ytsearch1:{title} {artist}"]
...
async def download_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    best_url = find_download_url(title, artist)
    if best_url: searches.append(best_url)
...
async def handle_link(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if "instagram.com" in text:
        ...
        file_path = f"yt_{user.id}.mp4"

        try:
            # ─── Cobalt API (بدون نیاز به IP مستقیم یوتیوب) ───
            COBALT_INSTANCES = [
                "https://api.cobalt.tools",
                "https://cobalt.api.timse.me",
                "https://co.wuk.sh",
            ]
            ...
            for instance in COBALT_INSTANCES:
                try:
                    r = requests.post(
                        f"{instance}/",
                        json={"url": text, "videoQuality": "720", "filenameStyle": "basic"},
                        headers={
                    ...
                    data = r.json()
                    logger.info(f"[Cobalt] {instance} → {data.get('status')}")

                    if data.get("status") in ("tunnel", "redirect", "stream"):
                        download_url = data.get("url")
                        video_title = data.get("filename", "YouTube Video").replace(".mp4", "")
                        break
                    elif data.get("status") == "picker":
                        # چند فایل برگشته — اولی رو بگیر
                        picker = data.get("picker", [])
                        if picker:
                            download_url = picker[0].get("url")
                            break
                except Exception as cobalt_err:
                    logger.warning(f"[Cobalt] {instance} خطا: {cobalt_err}")
                    continue
            ...
            if size > MAX_VIDEO_SIZE:
                await msg.edit_text(
                    f"❌ حجم ویدیو ({size // (1024*1024)} MB) بیشتر از سقف {MAX_VIDEO_MB} MB هست."
                )
                return
            ...
            add_download(user.id, "youtube")
            await msg.delete()

        except Exception as e:
            logger.error(f"[YouTube/Cobalt] {e}")
            await msg.edit_text(f"❌ خطا در دانلود:\n{str(e)[:300]}")
        finally:
            for f in glob.glob(f"yt_{user.id}.*"):
                try: os.remove(f)
                except: pass
