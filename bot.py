    ...
    if "instagram.com" in text:
        ...
        try:
            ...
            r = requests.get(
                ...
                headers=yt_headers,
                params={"videoUrl": text, "format": "mp4", "quality": "720p"},
                timeout=30
            )
            data = r.json()
            ...
            if not download_id:
                await msg.edit_text(f"❌ خطا: {data}")
                return
            ...
            await msg.edit_text("❌ دانلود از یوتیوب ممکن نشد. دوباره امتحان کن.")
    ...
    if is_audio:
        try:
            r = requests.get(
                "https://youtube-mp4-mp3-downloader.p.rapidapi.com/api/v1/download",
                headers=yt_headers,
                params={"videoUrl": url, "format": "mp3"},
                timeout=30
            )
        ...
        try:
            r = requests.get(
                "https://youtube-mp4-mp3-downloader.p.rapidapi.com/api/v1/download",
                headers=yt_headers,
                params={"videoUrl": url, "format": "mp4", "quality": f"{height}p"},
                timeout=30
            )
            download_id = r.json().get("id")
            if not download_id:
