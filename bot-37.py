...
def group_settings_keyboard(g):
    ...
    return InlineKeyboardMarkup([
        [btn("ضد اسپم", "anti_spam"), btn("ضد لینک", "anti_link")],
        [btn("ضد فوروارد", "anti_forward"), btn("ضد فحش", "anti_bad_words")],
...
async def handle_group_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_admin or user.id == ADMIN_ID: pass
        ...
        # ─── ضد فوروارد ───────────────────────────────────────
        if g.get("anti_forward") and (msg.forward_from or msg.forward_from_chat or msg.forward_date):
            try: await msg.delete()
            except: pass
            await chat.send_message(f"↩️ {user.first_name} فوروارد ممنوعه!")
            return
    ...
    if msg.text:
        ...
        # اگه "ربات" یا @username ربات در پیام بود
        if bot_name in text or (bot_username and f"@{bot_username}" in text):
            should_reply = True
