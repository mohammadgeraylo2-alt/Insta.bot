...
async def handle_voice(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        logger.info(f"[Shazam voice] status={r.status_code} body={r.text[:500]}")
...
async def handle_video(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        logger.info(f"[Shazam video] status={r.status_code} body={r.text[:500]} file_url={file.file_path}")
        track = r.json().get("track")
        if not track:
            await msg.edit_text("آهنگی شناسایی نشد"); return
        title = track.get("title", "نامشخص")
...
async def download_and_send(update, context, title, artist, msg):
    ...
    ydl_opts = {"format": "bestaudio/best", "outtmpl": f"song_{user_id}.%(ext)s", "cookiefile": "cookies.txt" if os.path.exists("cookies.txt") else None,
...
# ─── دانلود آهنگ از منابع متعدد ───────────────────────────────
def build_search_queries(title, artist):
    """ساخت لیست کوئری‌های متنوع برای پیدا کردن آهنگ‌های کمیاب"""
    queries = []
    full = f"{title} {artist}".strip()
    # SoundCloud - چند کوئری مختلف
    queries.append(f"scsearch1:{full}")
    if artist:
        queries.append(f"scsearch1:{title} {artist} official audio")
        queries.append(f"scsearch1:{artist} {title}")
    queries.append(f"scsearch1:{title}")
    # YouTube - چند کوئری مختلف
    queries.append(f"ytsearch1:{full}")
    queries.append(f"ytsearch1:{full} official audio")
    queries.append(f"ytsearch1:{full} lyrics")
    if artist:
        queries.append(f"ytsearch1:{title} {artist} full song")
    queries.append(f"ytsearch1:{title} audio")
    # YouTube Music
    queries.append(f"https://music.youtube.com/search?q={requests.utils.quote(full)}")
    return queries

def try_jiosaavn(title, artist, out_path):
    """جستجو و دانلود از JioSaavn (خوب برای آهنگ‌های ایرانی و هندی)"""
    try:
        ...
        logger.warning(f"[JioSaavn] {e}")
    return False

def try_deezer_preview(title, artist, out_path):
    """دانلود پیش‌نمایش ۳۰ ثانیه‌ای از Deezer (آخرین گزینه)"""
    try:
        query = f"{title} {artist}".strip()
        r = requests.get("https://api.deezer.com/search", params={"q": query, "limit": 5}, timeout=8)
        tracks = r.json().get("data", [])
        for track in tracks:
            preview = track.get("preview")
            if preview:
                resp = requests.get(preview, timeout=30, stream=True)
                if resp.status_code == 200:
                    with open(out_path, "wb") as f:
                        for chunk in resp.iter_content(chunk_size=8192):
                            f.write(chunk)
        ...
        logger.warning(f"[Deezer preview] {e}")
    return False, None

def download_song_multi_source(title, artist, user_id, url=None):
    """
    تلاش برای دانلود آهنگ از منابع متعدد به ترتیب اولویت.
    برمیگردونه: (موفق: bool, مسیر فایل: str, منبع: str)
    """
    mp3_path = f"song_{user_id}.mp3"
    tmp_path = f"song_{user_id}_tmp"
    ...
    # ۱. اگه URL مستقیم داریم، اول امتحان کن
    if url:
        try:
            with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                ydl.download([url])
            if os.path.exists(mp3_path) and os.path.getsize(mp3_path) > 10000:
                return True, mp3_path, "direct"
        except:
            pass

    # ۲. JioSaavn
    try:
        if try_jiosaavn(title, artist, tmp_path):
            import subprocess
            subprocess.run(["ffmpeg", "-y", "-i", tmp_path, "-q:a", "0", mp3_path], capture_output=True)
            if os.path.exists(tmp_path):
                os.remove(tmp_path)
            if os.path.exists(mp3_path) and os.path.getsize(mp3_path) > 10000:
                return True, mp3_path, "JioSaavn"
    except:
        pass

    # ۳. SoundCloud و YouTube با کوئری‌های متنوع
    ...
    # ۴. Deezer preview (۳۰ ثانیه) — آخرین گزینه
    ok, src = try_deezer_preview(title, artist, tmp_path)
    if ok:
        try:
            import subprocess
            subprocess.run(["ffmpeg", "-y", "-i", tmp_path, "-q:a", "0", mp3_path], capture_output=True)
            if os.path.exists(tmp_path):
                os.remove(tmp_path)
            if os.path.exists(mp3_path) and os.path.getsize(mp3_path) > 10000:
                return True, mp3_path, "Deezer (پیش‌نمایش ۳۰ ثانیه)"
        except:
            pass
...
async def download_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    url = track.get("url")
    msg = await query.message.reply_text(f"🔍 دارم دنبال آهنگ میگردم...\n🎵 {title} - {artist}")
    try:
        ok, mp3_path, source = download_song_multi_source(title, artist, user_id, url)
        if not ok:
            await msg.edit_text("❌ آهنگ پیدا نشد، حتی بعد از جستجو در چند منبع."); return
        caption = get_setting("caption") or CAPTION
        note = f"\n\n⚠️ فقط پیش‌نمایش ۳۰ ثانیه (کپی‌رایت)" if source and "پیش‌نمایش" in source else ""
        await query.message.reply_audio(
            audio=open(mp3_path, "rb"), title=title, performer=artist,
...
async def song_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        headers = {"x-rapidapi-key": RAPIDAPI_KEY, "x-rapidapi-host": host}
        r = requests.get(f"https://{host}/recognize/social/url", headers=headers, params={"url": file.file_path})
        logger.info(f"[Shazam song_callback] status={r.status_code} body={r.text[:500]} file_url={file.file_path}")
        track = r.json().get("track")
        if not track:
            await msg.edit_text("آهنگی پیدا نشد 😔"); return
        title = track.get("title", "نامشخص")
        artist = track.get("subtitle", "نامشخص")
        await msg.edit_text(f"🎵 {title} - {artist}\nدارم از چند منبع جستجو میکنم...")
        ok, mp3_path, source = download_song_multi_source(title, artist, user_id)
        if not ok:
            await msg.edit_text("❌ دانلود ممکن نشد، آهنگ در هیچ منبعی پیدا نشد."); return
        caption = get_setting("caption") or CAPTION
        note = f"\n\n⚠️ فقط پیش‌نمایش ۳۰ ثانیه (کپی‌رایت)" if source and "پیش‌نمایش" in source else ""
        await query.message.reply_audio(
            audio=open(mp3_path, "rb"), title=title, performer=artist,
...
async def handle_link(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if "instagram.com" in text:
        ...
        if "/stories/" in text:
            ...
            ydl_opts = {"outtmpl": f"video_{user.id}.mp4", "format": "best[ext=mp4]/best", "noplaylist": True, "cookiefile": "cookies.txt" if os.path.exists("cookies.txt") else None}
            try:
        ...
        try:
            apis = [
                {"host": "youtube-mp310.p.rapidapi.com", "info": "https://youtube-mp310.p.rapidapi.com/info"},
                {"host": "youtube-mp36.p.rapidapi.com", "info": "https://youtube-mp36.p.rapidapi.com/info"},
            ]
            r = None
            for api in apis:
                try:
                    yt_headers = {"x-rapidapi-key": RAPIDAPI_KEY, "x-rapidapi-host": api["host"]}
                    r = requests.get(api["info"], headers=yt_headers, params={"url": text}, timeout=30)
            ...
            keyboard = [
                [InlineKeyboardButton("🎥 720p", callback_data="yt_video_720"),
                 InlineKeyboardButton("📽 480p", callback_data="yt_video_480")],
                [InlineKeyboardButton("📹 360p", callback_data="yt_video_360")],
                [InlineKeyboardButton("🎵 فقط صدا (MP3)", callback_data="yt_audio_128")],
            ]
...
async def yt_quality_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    mp3_apis = [
        {"host": "youtube-mp310.p.rapidapi.com", "url": "https://youtube-mp310.p.rapidapi.com/mp3"},
        {"host": "youtube-mp36.p.rapidapi.com", "url": "https://youtube-mp36.p.rapidapi.com/mp3"},
    ]
    ...
    if is_audio:
        try:
            ...
            r = requests.get("https://yt-api.p.rapidapi.com/dl",
                             headers=headers, params={"id": yt_id}, timeout=60)
            d = r.json()

            dl_url = None
            all_formats = d.get("formats", []) + d.get("adaptiveFormats", [])
            if not all_formats and d.get("url"):
                dl_url = d.get("url")
            if not dl_url:
                for fmt in all_formats:
                    if fmt.get("hasAudio") and not fmt.get("hasVideo"):
                        dl_url = fmt.get("url")
                        break
            if not dl_url:
                for fmt in all_formats:
                    if fmt.get("hasAudio"):
                        dl_url = fmt.get("url")
                        break
            if not dl_url and all_formats:
                dl_url = all_formats[0].get("url")

            if not dl_url:
                raise Exception("لینک دانلود پیدا نشد")

            tmp_path = f"yt_audio_{user_id}.tmp"
            r2 = requests.get(dl_url, timeout=300, stream=True)
            ...
            for f in glob.glob(f"yt_audio_{user_id}.*"):
                try: os.remove(f)
                except: pass
        return
    else:
        ...
        try:
            # دانلود ویدیو با youtube-mp310
            mp3_apis = [
                {"host": "youtube-mp310.p.rapidapi.com", "url": "https://youtube-mp310.p.rapidapi.com/mp4"},
                {"host": "youtube-mp36.p.rapidapi.com", "url": "https://youtube-mp36.p.rapidapi.com/mp4"},
            ]
            video_url = None
            last_err = None
            for api in mp3_apis:
                try:
                    yt_h = {"x-rapidapi-key": RAPIDAPI_KEY, "x-rapidapi-host": api["host"]}
                    r = requests.get(api["url"], headers=yt_h, params={"url": url, "quality": f"{height}p"}, timeout=60)
                    d = r.json()
                    video_url = d.get("url") or d.get("link") or d.get("download_url")
                    if video_url:
                        break
                    last_err = d
                except Exception as e:
            ...
            if not video_url:
                raise Exception(f"فرمت ویدیو پیدا نشد - {last_err}")
