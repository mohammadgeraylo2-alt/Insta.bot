...
async def yt_quality_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_audio:
        try:
            ...
            # اول RapidAPI امتحان کن
            dl_url = None
            for api_host in ["youtube-mp310.p.rapidapi.com", "youtube-mp36.p.rapidapi.com"]:
                try:
                    headers = {"x-rapidapi-key": RAPIDAPI_KEY, "x-rapidapi-host": api_host}
                    r = requests.get(f"https://{api_host}/mp3", headers=headers, params={"url": url}, timeout=60)
                    d = r.json()
                    dl_url = d.get("link") or d.get("url") or d.get("download")
                    if dl_url:
                        break
                except:
                    continue
        ...
        try:
            ...
            for api_host in ["youtube-mp310.p.rapidapi.com", "youtube-mp36.p.rapidapi.com"]:
                try:
                    headers = {"x-rapidapi-key": RAPIDAPI_KEY, "x-rapidapi-host": api_host}
                    r = requests.get(f"https://{api_host}/mp4", headers=headers,
                                     params={"url": url, "quality": height}, timeout=60)
                    d = r.json()
                    dl_url = d.get("link") or d.get("url") or d.get("download")
                    if dl_url:
                        break
