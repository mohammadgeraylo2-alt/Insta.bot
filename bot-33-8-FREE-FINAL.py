...
# ─── ساخت عکس: Gemini → HuggingFace FLUX → Pollinations ─────────────────
async def ai_generate_image(prompt):
    ...
    negative = (
        "blurry, low quality, worst quality, watermark, text, logo, "
        "extra fingers, deformed face, distorted eyes, cropped, ugly"
    )

    try:
        encoded = urllib.parse.quote(
            enhanced + ", ultra detailed, photorealistic, 8k, cinematic lighting"
        )
        ...
        return None, f"Pollinations Error: {r.status_code}"

    except Exception as e:
        return None, str(e)

# ─── ادیت عکس با Google Gemini 2.5 Flash Image API ───────────────────────

async def ai_edit_image(img_bytes, instruction):
    try:
        if not GEMINI_KEY:
            return None, "کلید GEMINI_KEY تنظیم نشده. از https://aistudio.google.com/apikey کلید رایگان بگیر و توی Railway اضافه کن."
        ...
        return None, "عکسی در پاسخ Gemini پیدا نشد."
    except Exception as e:
        return None, str(e)
