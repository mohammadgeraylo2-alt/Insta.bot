...
async def download_and_send(update, context, title, artist, msg):
    ...
    # بررسی اینکه آیا ffprobe واقعی داریم یا نه
    _has_ffprobe = False
    try:
        import imageio_ffmpeg
        _ff = imageio_ffmpeg.get_ffmpeg_exe()
        _fp = os.path.join(os.path.dirname(_ff), "ffprobe")
        if os.path.exists(_fp) and os.path.realpath(_fp) != os.path.realpath(_ff):
            _has_ffprobe = True
        else:
            import shutil
    ...
    if _has_ffprobe:
        ...
        # حالت بدون ffprobe — دانلود مستقیم m4a/opus بدون تبدیل
        ydl_opts = {
    ...
    # پیدا کردن فایل دانلود شده (هر پسوندی که باشه)
    audio_file = mp3_path if os.path.exists(mp3_path) else None
    if not audio_file:
        found = glob.glob(f"song_{user_id}.*")
        if found:
            audio_file = found[0]
...
async def download_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    mp3_path = f"song_{user_id}.mp3"
    ydl_opts = {"format": "bestaudio[ext=m4a]/bestaudio[ext=opus]/bestaudio/best",
                "outtmpl": f"song_{user_id}.%(ext)s",
    ...
    audio_file = mp3_path if os.path.exists(mp3_path) else (glob.glob(f"song_{user_id}.*") or [None])[0]
    if not audio_file:
        await msg.edit_text("❌ فایل آهنگ پیدا نشد."); return
    caption = get_setting("caption") or CAPTION
    try:
